...
class ClientMessageTests {
    ...
    private val secondFilter = NostrFilter(
        secondEventIdList, secondAuthorList, secondKindList, referencedEventIdList,
    ...
    fun `the request message for the first filter is correctly serialized`() {
        val correctRequestJson =
            """["REQ","mySub",{"ids":["event_id_1","event_id_2","event_id_3"],"authors":["author_pubkey_1","author_pubkey_2"],"kinds":[1],"#e":["ref_event_id_1","ref_event_id_2"],"#p":["ref_pubkey_1"],"since":1653736339,"until":1653822739,"limit":25}]"""
    ...
    fun `the request message for the second filter is correctly serialized`() {
        val correctRequestJson =
            """["REQ","mySub",{"ids":["event_id_4","event_id_5"],"authors":["author_pubkey_3","author_pubkey_4"],"kinds":[0,2],"#e":["ref_event_id_3","ref_event_id_4"],"#p":["ref_pubkey_2","ref_pubkey_3","ref_pubkey_4"],"since":1653736339,"until":1653822739,"limit":10}]"""

        val requestMessage =
            RequestMessage(subscriptionId = "mySub", filters = listOf(secondFilter))
        val requestJson = testEventMapper.writeValueAsString(requestMessage)
    ...
    fun `the request with several filters is correctly serialized`() {
        ...
        val requestMessage =
            RequestMessage(subscriptionId = "mySub", filters = listOf(filter, secondFilter))
