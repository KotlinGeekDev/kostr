...
class SchnorrSigTest {
    ...
    fun testAllVectors() {
        ...
        if (null != tv.secKey) {
            // test getPubKey
            val pubKeyActual = CryptoUtils.getPublicKey(tv.secKey!!)
            assertEquals("Failed reproducing the pubKey", Hex.encode(tv.pubKey), Hex.encode(pubKeyActual))
            // test sign
            val sigActual = CryptoUtils.signContent(tv.secKey!!, tv.msg)
            assertArrayEquals("Failed signing test ${tv.id} (${tv.comment})", tv.sig, sigActual)
        }
        val resultActual = CryptoUtils.verifyContentSignature(tv.sig, tv.pubKey, tv.msg)
...
16,,02DFF1D77F2A671C5F36183726DB2341BE58FEAE1DA2DECED843240F7B502BA659,243F6A8885A308D313198A2E03707344A4093822299F31D0082EFA98EC4E6C89,2A298DACAE57395A15D0795DDBFD1DCB564DA82B0F269BC70A74F8220429BA1DFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141,FALSE,"sig[32:64] is equal to curve order""""
                ...
                .map {
                    ...
                    TV(
                        parts[0].toInt(),
                        if (parts[1].isNotEmpty()) Hex.decode(parts[1]) else null,
                        Hex.decode(parts[2]),
                        Hex.decode(parts[3]),
                        Hex.decode(parts[4]),
                        parts[5] == "TRUE",
